"""
CT decomposition and rendering for real 3D-Front scenes.
...
Usage (CLI):
  python -m raw_optimizer.real_scene_ct render \
      --scene datasets/3D-Front/00ad8345-45e0-45b3-867d-4a3c88c2517a/0 \
      --out real_scene_results/00ad8345/rendered \
      --az 45 --shader ct_sh

  python -m raw_optimizer.real_scene_ct decompose \
      --scene datasets/3D-Front/00ad8345-45e0-45b3-867d-4a3c88c2517a/0 \
      --out real_scene_results/00ad8345/ct_sh \
...
def make_proxy_geometry(
    ...
    Since the real scene has no depth map, fragment positions are approximated
    by placing the camera at (0, 0, cam_dist) and distributing pixel positions
    on the z=0 plane according to fov_deg (perspective).  This gives correct
    view-direction variation across the image without actual depth.
    """
...
def render_scene(
    ...
    """Render the scene using GT material maps and a single directional light.
...
def _build_parser():
    p = argparse.ArgumentParser(description="CT decomp/render for real 3D-Front scenes")
    sub = p.add_subparsers(dest="cmd", required=True)

    # render sub-command
    r = sub.add_parser("render", help="Render scene from GT maps + directional SH")
    ...
    d = sub.add_parser("decompose", help="Decompose scene: estimate albedo + lighting")
