#!/usr/bin/env python
"""
scripts/sweep_cfg.py — parallel hyperparameter search for CT-SH decomposition.

Sibling to scripts/decompose_batch.py. Takes the notebook's self-contained `datasets/`
...
kept identical to scripts/decompose_batch.py so results are directly comparable.
...
Example (VM, run the whole grid 8-wide):
  WANDB_API_KEY=... python scripts/sweep_cfg.py \
...
# ───────────────────────── plotting (mirrors scripts/decompose_batch.py) ────────────
...
def build_parser():
    ...
    # ── fixed base cfg (mirrors scripts/decompose_batch.py) ──
