...
from raw_renderer.shaders.lighting      import PointLight, EnvMap, SHLighting
from raw_renderer.shaders.phong         import PhongMaterial
from raw_renderer.shaders.cook_torrance import PBRMaterial
...
def _rasterize(sverts, faces, W, H, face_batch=32):
    ...
    for fi in range(0, F_n, face_batch):
        ...
        ax = sverts[fb[:, 0], 0]; ay = sverts[fb[:, 0], 1]; az = sverts[fb[:, 0], 2]
        bx = sverts[fb[:, 1], 0]; by = sverts[fb[:, 1], 1]; bz = sverts[fb[:, 1], 2]
        cx = sverts[fb[:, 2], 0]; cy = sverts[fb[:, 2], 1]; cz = sverts[fb[:, 2], 2]

        denom = (by - cy)*(ax - cx) + (cx - bx)*(ay - cy)   # (B,)
        ok    = denom.abs() > 1e-6

        # barycentric coords for all pixels × batch faces  →  (P, B)
        w0 = ((by-cy)*(pcx[:,None]-cx) + (cx-bx)*(pcy[:,None]-cy)) / (denom + 1e-10)
        w1 = ((cy-ay)*(pcx[:,None]-cx) + (ax-cx)*(pcy[:,None]-cy)) / (denom + 1e-10)
        w2 = 1.0 - w0 - w1
        ...
        bary     = torch.where(imp[:, None],
                       torch.stack([w0[aP, bl], w1[aP, bl], w2[aP, bl]], 1),
                       bary)
...
def _interp(verts, fn, vn, faces, face_ids, bary, hit, smooth):
    ...
    frag_pos = bw[:,0:1]*verts[i0] + bw[:,1:2]*verts[i1] + bw[:,2:3]*verts[i2]
...
def _phong_point(frag_pos, N, cam_pos, mat, light):
    dev = frag_pos.device
    lp  = _cuda(light.position, dev)
    lc  = _cuda(light.color,    dev)
    bc  = _cuda(mat.base_color, dev)

    V   = _norm(cam_pos - frag_pos)
    L   = _norm(lp - frag_pos)
    NdL = (N*L).sum(1, keepdim=True).clamp(min=0)

    diff = mat.kd * lc * bc / torch.pi * NdL
    R    = _norm(2*NdL*N - L)
    RdV  = (R*V).sum(1, keepdim=True).clamp(min=0)
    spec = mat.ks * lc * RdV**mat.shininess
    amb  = mat.ka * lc * bc

    return (amb + diff + spec).clamp(0, 1)


def _phong_envmap(frag_pos, N, cam_pos, mat, light, sbatch=128):
    dev  = frag_pos.device
    bc   = _cuda(mat.base_color, dev)
    dirs = _cuda(light._dirs,        dev)
    rad  = _cuda(light._image_flat,  dev)
    dw   = _cuda(light._solid_angles,dev)
    S    = dirs.shape[0]
    ...
    for si in range(0, S, sbatch):
        L_b  = dirs[si:si+sbatch]                 # (B, 3)
        r_b  = rad[si:si+sbatch]
        dw_b = dw[si:si+sbatch]                   # (B,)
        ...
        # RdV = dot(2*NdL*N - L, V) = 2*NdL*NdV - L·V  (no (M,B,3) needed)
        LdV = V @ L_b.T                           # (M, B)
        RdV = (2.0*NdL*NdV - LdV).clamp(min=0) * mask
        spec += (RdV**mat.shininess * dw_b) @ r_b

    mean_rad = rad.mean(0)
    return (mat.ka*mean_rad*bc + mat.kd*diff*bc/torch.pi + mat.ks*spec*norm_f).clamp(0,1)


def _phong_sh(frag_pos, N, cam_pos, mat, light):
    dev     = frag_pos.device
    coeffs  = _cuda(light.coeffs, dev)            # (9, 3)
    bc      = _cuda(mat.base_color, dev)

    irr  = _sh_irradiance(coeffs, N)
    diff = mat.kd * irr * bc / torch.pi
...
def _ct_point(frag_pos, N, cam_pos, mat, light):
    dev    = frag_pos.device
    ...
    spec = F * (D*G / (4*NdV_h + 1e-7))[:, None] * lc
    k_d  = (1 - F) * (1 - mat.metallic)
    diff = k_d * albedo / torch.pi * lc * NdL_h[:, None]

    out[hit] = (diff + spec).clamp(0, 1)
    return out


def _ct_envmap(frag_pos, N, cam_pos, mat, light, sbatch=64):
    dev    = frag_pos.device
    albedo = _cuda(mat.albedo,        dev)
    dirs   = _cuda(light._dirs,       dev)
    rad    = _cuda(light._image_flat, dev)
    ...
    for si in range(0, S, sbatch):
        ...
        spec     += (F * w[:, :, None] * r_b).sum(1)           # (M, 3)
        diff_irr += ((NdL*dw_b*mf) @ r_b)                      # (M, 3)
        F_sum    += (F * mf[:, :, None]).sum(1)
        n_valid  += mf.sum(1)

    F_mean = F_sum / n_valid[:, None].clamp(min=1)
    k_d    = (1 - F_mean) * (1 - mat.metallic)
    diff   = k_d * albedo / torch.pi * diff_irr
    return (diff + spec).clamp(0, 1)


def _ct_sh(frag_pos, N, cam_pos, mat, light):
    dev    = frag_pos.device
    albedo = _cuda(mat.albedo, dev)
...
def render(
    mesh:        Mesh,
    camera:      Camera,
    material,                 # PhongMaterial | PBRMaterial
    light,                    # PointLight | EnvMap | SHLighting
    width:       int  = 512,
    height:      int  = 512,
    smooth:      bool = False,
    ...
    if hit.any():
        frag_pos, N = _interp(verts, fn, vn, faces, face_ids, bary, hit, smooth)

        if isinstance(material, PhongMaterial):
            if   isinstance(light, PointLight): col = _phong_point(frag_pos, N, cam_t, material, light)
            elif isinstance(light, EnvMap):     col = _phong_envmap(frag_pos, N, cam_t, material, light)
            elif isinstance(light, SHLighting): col = _phong_sh(frag_pos, N, cam_t, material, light)
            else: raise TypeError(type(light))
        elif isinstance(material, PBRMaterial):
            if   isinstance(light, PointLight): col = _ct_point(frag_pos, N, cam_t, material, light)
            elif isinstance(light, EnvMap):     col = _ct_envmap(frag_pos, N, cam_t, material, light)
            elif isinstance(light, SHLighting): col = _ct_sh(frag_pos, N, cam_t, material, light)
            else: raise TypeError(type(light))
        else:
