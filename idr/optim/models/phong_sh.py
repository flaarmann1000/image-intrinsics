"""Phong + spherical-harmonics lighting.
...
from idr.config import _PHONG_SH_PARAMS, SHININESS_RANGE
from idr.render import shade_phong_sh
from idr.render.ops import _norm
from idr.optim.transforms import pad_sh
...
def _optimize_phong_sh(
    ...
    gt_map_grad = {
        ...
        "sh":        np.stack([pad_sh(s, 9) for s in gt_sh_coeffs]).astype(np.float32) if gt_sh_coeffs is not None else None,
